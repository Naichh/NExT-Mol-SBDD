...
def process_full_protein(protein_pdb_path, model, device):
    """
    处理一个完整的蛋白质PDB文件，返回完整的embedding和从PDB直接解析的残基ID列表。
    """
    try:
        # 步骤 1: 我们自己从PDB文件解析出残基ID列表，作为唯一真理
        full_protein_res_keys = get_residue_ids_from_pdb(protein_pdb_path)
        if not full_protein_res_keys:
            return None, None

        # 步骤 2: 使用ESM SDK加载PDB并生成embedding
        ...
        # 步骤 3: 验证embedding长度和我们解析出的残基数量是否一致
        if full_embedding.shape[0] != len(full_protein_res_keys):
             print(f"\n[!] WARNING: Embedding长度 ({full_embedding.shape[0]}) 与解析出的残基数 ({len(full_protein_res_keys)}) 不匹配！ 文件: {protein_pdb_path}")
             return None, None
...
def main(args):
    ...
    for protein_fn, pocket_fns in tqdm(protein_to_pockets_map.items(), desc=f"{shard_info} 处理中", position=args.shard_id):
        ...
        # 创建从 (链ID, 残基ID) 到 embedding 索引的映射
