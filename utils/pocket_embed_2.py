import os
import torch
import pickle  # --- 修改 1: 导入 pickle 库 ---
import tempfile
import argparse
...
def main(pdb_path, sdf_path, output_dir, radius=10.0, device="cuda"):
    ...
    # --- 修改 2: 输出文件扩展名改为 .pkl ---
    embed_path = os.path.join(output_dir, f"{file_name}.pkl")
    ...
    # 1. 将 PyTorch 张量移动到 CPU 并转换为 NumPy 数组
    embedding_numpy = full_embedding.cpu().detach().numpy()
    
    # 2. 使用 pickle 将 NumPy 数组写入文件
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract FULL protein embedding using ESM3 and save as .pkl")
