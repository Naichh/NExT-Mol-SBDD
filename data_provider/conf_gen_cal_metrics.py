...
def single_conf_gen(tgt_mol: Chem.Mol, num_confs: int = 1000, seed: int = 42, mmff: bool = False, randomize_angles: bool = False, threads: int = 0, removeHs: bool = True, addHs: bool = True) -> Chem.Mol:
    ...
    allconformers = AllChem.EmbedMultipleConfs(
        mol, numConfs=num_confs, randomSeed=seed, clearConfs=True, numThreads=threads, maxAttempts=1000,
    )
    if len(allconformers) == 0:
        AllChem.Compute2DCoords(mol)
        allconformers = mol.GetConformers()
...
def clustering(
    ...
    rdkit_mol = single_conf_gen(mol, num_confs=int(M // 4), seed=seed, threads=threads, removeHs=removeHs, addHs=addHs)
    if removeHs:
    ...
    if mmff:
        rdkit_mol = single_conf_gen(mol, num_confs=M, mmff=True, seed=seed+1, threads=threads, removeHs=removeHs, addHs=addHs)
        if removeHs:
            rdkit_mol = Chem.RemoveHs(rdkit_mol)
        sz = len(rdkit_mol.GetConformers())
        for i in range(sz):
    ...
    if randomized_angles:
        rdkit_mol = single_conf_gen(mol, num_confs=int(M // 4), seed=seed+2, threads=threads, randomize_angles=True, removeHs=removeHs, addHs=addHs)
        if removeHs:
            rdkit_mol = Chem.RemoveHs(rdkit_mol)
        sz = len(rdkit_mol.GetConformers())
        for i in range(sz):
...
def generate_conformers(
    ...
    # add forcefield optimized conformers
    if mmff:
        rdkit_mol = single_conf_gen(mol, num_confs=M-int(M//4), mmff=True, seed=seed+1, threads=threads, removeHs=removeHs, addHs=addHs)
        if removeHs:
            rdkit_mol = Chem.RemoveHs(rdkit_mol)
        sz = len(rdkit_mol.GetConformers())
        for i in range(sz):
    ...
    if use_clustering:
        rdkit_coords_flatten = np.stack(rdkit_coords_list, axis=0).reshape(total_sz, -1)
        clust = KMedoids(n_clusters=N, random_state=seed, )
        clust.fit(rdkit_coords_flatten)
        idxs = clust.medoid_indices_.tolist()
        rdkit_coords_list = [rdkit_coords_list[idx] for idx in idxs]
    return rdkit_coords_list
...
def my_single_process_data(tgt_mol) -> List:
    if False:
        M = min(20 * len(tgt_mol_list), 2000)
        N = 2 * len(tgt_mol_list)
    else:
        M = 20
        N = 10
    tgt_mol = copy.deepcopy(tgt_mol)
    tgt_mol.RemoveAllConformers()
    # tgt_mol = Chem.RemoveHs(tgt_mol)
    rdkit_cluster_coords_list = clustering(tgt_mol, M=M, N=N, removeHs=False, addHs=False)
    return rdkit_cluster_coords_list


def my_fast_single_process_data(tgt_mol) -> List:
    M = 20
    N = 10
    tgt_mol = copy.deepcopy(tgt_mol)
    tgt_mol.RemoveAllConformers()
...
def write_lmdb(content_list, output_dir, name, nthreads=16):
    ...
    with Pool(nthreads) as pool:
        i = 0
        for inner_output in tqdm(pool.imap(inner_process, content_list), total=len(content_list)):
