...
class VinaDockingTask(BaseDockingTask):
    ...
    #     return cls(protein_path, ligand_rdmol, **kwargs)
    @classmethod
    def from_generated_mol(cls, mol, pocket_path, protein_root, **kwargs):


        full_protein_pocket_path = os.path.join(protein_root, pocket_path)

        if not os.path.exists(full_protein_pocket_path):
            raise FileNotFoundError(f"Receptor pocket file not found: {full_protein_pocket_path}")
            
        #    第一个参数是蛋白质路径，第二个是配体mol对象
        return cls(full_protein_pocket_path, mol, **kwargs)



    def __init__(self, protein_path, ligand_rdmol, tmp_dir='/tmp/nai/vina/', center=None,
    ...
    def run(self, mode='dock', exhaustiveness=8, **kwargs):
        ...
        # if mode in ['score_only', 'minimize']:
        #     # 对于打分和局部优化，使用基于配体中心的快速盒子（符合TargetDiff论文逻辑）
        #     dock.pocket_center, dock.box_size = self.center, [self.size_x, self.size_y, self.size_z]
        # elif mode == 'dock':
        #     # 对于全局对接，必须使用基于蛋白质口袋的正确盒子
        #     dock.get_box(ref=protein_pdbqt, buffer=5.0) # buffer值可按需调整
        # else:
        #     raise ValueError(f"Unknown Vina mode: {mode}")
        # score, pose = dock.dock(score_func='vina', mode=mode, exhaustiveness=exhaustiveness, save_pose=True, **kwargs)
        # return [{'affinity': score, 'pose': pose}]
